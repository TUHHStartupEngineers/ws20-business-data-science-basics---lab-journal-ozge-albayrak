...
# Get URL for families in the categories and append them to original URL (url_home)
rosebike_family_css <- rosebike_family_tbl %>%
  pull(category) %>%
  stringr::str_c(collapse = ", ")
...
bike_category_tbl <- html_home %>%
  ...
  # Select nodes by the category
  html_nodes(css = rosebike_family_css) %>%
